...
export default async function TenantPage({ params }: { params: Promise<{ locale: string; tenantId: string }> }) {
  ...
  return <>
    ...
    {access.status === 'authorized' ? <><h2>{access.name}</h2><p>{ar ? 'تم التحقق من هويتك وعضويتك الحالية.' : 'Your identity and current membership are verified.'}</p><p>{ar ? 'سير العمل التشغيلي غير متاح في هذا الإصدار.' : 'Operational workflows are not available in this release.'}</p></> : <p role="alert">{access.status === 'mfa-required' ? (ar ? 'يتطلب وصول المالك جلسة تحقق بخطوتين. لم يتم تحميل بيانات المؤسسة. تواصل مع المسؤول لإعداد الوصول.' : 'Owner access requires a session verified with MFA. Organization data has not been loaded. Contact your administrator to arrange access.') : access.status === 'setup-needed' ? (ar ? 'الوصول يحتاج إلى إعداد من المسؤول.' : 'Access needs administrator configuration.') : (ar ? 'تعذر التحقق من الوصول. لم يتم تحميل بيانات المؤسسة.' : 'Access could not be verified. Organization data has not been loaded.')}</p>}
