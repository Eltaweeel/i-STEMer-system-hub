...
vi.mock('server-only', () => ({}));
import { startContentCalendarWorkerService } from '../lib/workflow/content-calendar-worker-service';
import type { ContentCalendarCommandPort } from '../lib/workflow/content-calendar-worker';

function fakeConfig(claim: ContentCalendarCommandPort['claim']) {
  return {
    port: { claim, fail: vi.fn(), complete: vi.fn() },
    dispatch: vi.fn(),
    endpoint: { hostname: '127.0.0.1', port: 4200 },
    keyId: 'worker-key-1',
...
describe('startContentCalendarWorkerService', () => {
  ...
  it('runs a cycle on every tick and reports each result via onCycle', async () => {
    ...
    const onCycle = vi.fn();
    const service = startContentCalendarWorkerService({ config: fakeConfig(claim), intervalMs: 1000, onCycle });
  ...
  it('never starts a new cycle while one is still in flight, even across several ticks', async () => {
    ...
    const service = startContentCalendarWorkerService({ config: fakeConfig(claim), intervalMs: 100 });
  ...
  it('stops scheduling further cycles once stopped, and running reflects that', async () => {
    const claim = vi.fn().mockResolvedValue(null);
    const service = startContentCalendarWorkerService({ config: fakeConfig(claim), intervalMs: 1000 });
  ...
  it('calling stop twice is a harmless no-op', async () => {
    const service = startContentCalendarWorkerService({ config: fakeConfig(vi.fn().mockResolvedValue(null)), intervalMs: 1000 });
    service.stop();
  ...
  it('reports a rejecting cycle via onCycleError instead of throwing, and keeps running', async () => {
    ...
    const onCycle = vi.fn();
    const service = startContentCalendarWorkerService({ config: fakeConfig(claim), intervalMs: 100, onCycle, onCycleError });
