...
import type { ResearchCommandPort } from '../lib/workflow/research-worker';
...
describe('startResearchWorkerService', () => {
  ...
  it('runs a cycle on every tick and reports each result via onCycle', async () => {
    ...
    const onCycle = vi.fn();
    const service = startResearchWorkerService({ config: fakeConfig(claim), intervalMs: 1000, onCycle });
  ...
  it('stops scheduling further cycles once stopped, and running reflects that', async () => {
    const claim = vi.fn().mockResolvedValue(null);
    const service = startResearchWorkerService({ config: fakeConfig(claim), intervalMs: 1000 });
  ...
  it('calling stop twice is a harmless no-op', async () => {
    const service = startResearchWorkerService({ config: fakeConfig(vi.fn().mockResolvedValue(null)), intervalMs: 1000 });
    service.stop();
  ...
  it('reports a rejecting cycle via onCycleError instead of throwing, and keeps running', async () => {
    ...
    const onCycle = vi.fn();
    const service = startResearchWorkerService({ config: fakeConfig(claim), intervalMs: 100, onCycle, onCycleError });
