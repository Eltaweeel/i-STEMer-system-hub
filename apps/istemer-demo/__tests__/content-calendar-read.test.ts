...
import { GET as retrieve } from '../app/api/workflows/content-calendar/[runId]/route';
...
beforeEach(() => {
  ...
  boundary.level = 'aal2';
  boundary.rows = {
    content_calendar_tasks: [{ id: taskId, tenant_id: tenantId, requester_id: requesterId, run_id: runId, brief_revision_id: briefRevisionId }],
    agent_runs: [{ id: runId, tenant_id: tenantId, mode: 'content_calendar', state: 'queued' }],
  };
});
afterEach(() => vi.unstubAllEnvs());

describe('content-calendar retrieval HTTP boundary', () => {
  const get = () => retrieve(new Request(`https://staging.example.org/api/workflows/content-calendar/${runId}`), { params: Promise.resolve({ runId }) });
  it('reads queued state without fabricating an artifact or attempt', async () => {
    const response = await get();
    expect(response.status).toBe(200); expect(response.headers.get('cache-control')).toBe('private, no-store');
    expect(await response.json()).toEqual({ contractVersion: 'content-calendar.v1', runId, taskId, status: 'queued',
  ...
  it.each(['tenant_id', 'requester_id'])('denies a run owned by another %s', async (field) => {
    boundary.rows.content_calendar_tasks![0]![field] = attemptId;
    expect((await get()).status).toBe(404);
  });
  it('reports persisted failures with explicit retry eligibility', async () => {
    boundary.rows.agent_runs![0]!.state = 'failed';
    boundary.rows.content_calendar_attempts = [{ id: attemptId, tenant_id: tenantId, task_id: taskId,
    ...
    boundary.rows.content_calendar_attempts[0]!.retry_requested_at = '2026-09-16T12:00:00Z';
  ...
  it('returns the committed revision and rejects mutated identity or lineage', async () => {
    ...
    boundary.rows.content_calendar_outcomes = [{ tenant_id: tenantId, task_id: taskId, artifact_revision_id: revisionId }];
    boundary.rows.content_calendar_revision_bodies = [
    ...
    // Every entry shares the artifact's binding (enforced by the contract's own
    ...
    const rebind = (patch: Partial<ReturnType<typeof artifactFixture>>) => ({ ...artifact, ...patch,
      entries: artifact.entries.map((entry) => ({ ...entry, ...patch })) });
    for (const change of [rebind({ runId: attemptId }), rebind({ attemptId: taskId }), rebind({ sourceRevisionId: taskId })]) {
      boundary.rows.content_calendar_revision_bodies[1]!.body = change;
